...
# Known n8n-workflow constants that need resolution
...
TEMPLATE_PATH = CURRENT_DIR / "n8n_Node_Default_Settings_Template.md"
...
class TypeScriptParser:
    ...
    def extract_exported_array(content: str, array_name: str, source_path: Optional[Path] = None) -> List[Dict[str, Any]]:
        ...
        Example: export const description: SheetProperties = [...]
        Args:
            content: TypeScript file content
        ...
        """
        # Pattern to find the exported array - make type annotation optional/flexible
        # Matches: export const name: Type[] = [  OR  export const name: Type = [
    ...
    @staticmethod
    def extract_description_object(content: str, const_name: str, source_path: Optional[Path] = None) -> Dict[str, Any]:
        """Extract properties, credentials, and metadata from a description object export.

        Args:
            content: The TypeScript file content
            const_name: Name of the const to extract (e.g., 'versionDescription')
            source_path: Path to the file (for resolving imports and spreads)
        """
        ...
        if props_match:
            ...
            if depth == 0:
                ...
                # Parse properties and collect spreads
    ...
    def extract_string_value(content: str, field: str) -> Optional[str]:
        ...
        # Resolve known workflow constants
        if result_str and result_str in KNOWN_WORKFLOW_CONSTANTS:
            result_str = KNOWN_WORKFLOW_CONSTANTS[result_str]
    ...
    @staticmethod
    def _split_into_objects(text: str) -> Tuple[List[str], List[Dict[str, Any]]]:
        """
        Split a string into individual object strings and spread operators
        Returns: (objects, spreads) where spreads are dicts with 'var' and optional 'map_transform'
        """
        objects = []
        spreads = []
        depth = 0
        ...
        while i < len(text):
            ...
            # Check for spread operator with .map() transformation
            if char == '.' and i + 2 < len(text) and text[i+1:i+3] == '..':
                # Check for .map() pattern: ...arrayName.map(...)
                map_match = re.match(r'\.\.\.(\w+(?:\.\w+)*)\.map\s*\(', text[i:])
                if map_match:
                    base_array = map_match.group(1)
                    # Extract the map transformation function
                    paren_depth = 0
                    j = i + len(map_match.group(0))
                    map_start = j
                    while j < len(text):
                        if text[j] == '(':
                            paren_depth += 1
                    ...
                    map_transform = text[map_start:j].strip()
                    spreads.append({
                        'var': base_array,
                ...
                # Check for regular spread operator (handles both ...variable and ...module.property)
                ...
                if spread_match:
                    ...
                    spreads.append({'var': spread_var})  # Store as dict for consistency
    ...
    @staticmethod
    def _apply_map_transformation(properties: List[Dict[str, Any]], map_transform: str) -> List[Dict[str, Any]]:
        """
        Apply a .map() transformation to properties based on the transformation code
        Currently handles common patterns like adding displayOptions.show conditions
        """
        transformed = []

        # Pattern: (prop) => ({...prop, displayOptions: {...prop.displayOptions, show: {..., '@tool': [true]}}})
        # This adds '@tool': [true] to the show conditions
        if "'@tool'" in map_transform or '"@tool"' in map_transform:
            for prop in properties:
                # Create a copy of the property
                new_prop = prop.copy()

                # Add or modify displayOptions
                if 'displayOptions' not in new_prop:
                    new_prop['displayOptions'] = {}
                if not isinstance(new_prop['displayOptions'], dict):
                    new_prop['displayOptions'] = {}
                if 'show' not in new_prop['displayOptions']:
                    new_prop['displayOptions']['show'] = {}
                if not isinstance(new_prop['displayOptions']['show'], dict):
                    new_prop['displayOptions']['show'] = {}

                # Add the @tool condition
                new_prop['displayOptions']['show']['@tool'] = [True]
                transformed.append(new_prop)
            return transformed

        # Default: return properties unchanged if we don't recognize the transformation
        return properties

    @staticmethod
    def _resolve_spreads(spread_vars: List[Dict[str, Any]], imports: Dict[str, List[str]], node_path: Path, current_content: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Resolve spread operators by loading and parsing the imported or local arrays
        Args:
            spread_vars: List of spread dicts with 'var' and optional 'map_transform'
        ...
        for spread_item in spread_vars:
            # Extract spread variable name and optional transformation
            if isinstance(spread_item, dict):
                spread_var = spread_item.get('var', '')
                map_transform = spread_item.get('map_transform')
            else:
                # Fallback for backward compatibility (shouldn't happen with new code)
                spread_var = spread_item
                map_transform = None
            ...
            if '.' in spread_var:
                ...
                property_path = parts[1]  # First property after module (e.g., "description")
                ...
                if source_file and Path(source_file).exists():
                    try:
                        ...
                        # Try extract_exported_array for the property
                        props = TypeScriptParser.extract_exported_array(content, property_path, Path(source_file))

                        if not props:
                            # Try as object property (for namespace.description patterns)
                            single_prop = TypeScriptParser.extract_exported_property(content, property_path)
                            if single_prop:
                                if map_transform:
                                    transformed = TypeScriptParser._apply_map_transformation([single_prop], map_transform)
                                    if transformed:
                                        resolved_properties.extend(transformed)
                                else:
                        ...
                        if props:
                            if map_transform:
                                props = TypeScriptParser._apply_map_transformation(props, map_transform)
            ...
            if current_content:
                ...
                if local_props:
                    if map_transform:
                        local_props = TypeScriptParser._apply_map_transformation(local_props, map_transform)
    ...
    def _parse_single_property(obj_str: str) -> Optional[Dict[str, Any]]:
        ...
        # Parse options (for options, multiOptions types)
        if prop.get('type') in ['options', 'multiOptions']:
            options_match = re.search(r"options:\s*\[([\s\S]*?)\](?=\s*[,}])", obj_str)
            if options_match:
                prop['options'] = TypeScriptParser._parse_options(options_match.group(1))

        # Parse collection type with RECURSIVE nested options extraction
        if prop.get('type') == 'collection':
            # Find options array using bracket-depth counting (not regex)
            options_start = obj_str.find('options:')
            if options_start != -1:
                # Find the opening bracket
                bracket_start = obj_str.find('[', options_start)
                if bracket_start != -1:
                    ...
                    if depth == 0:
                        ...
                        options_content = obj_str[bracket_start + 1:i - 1]
                        nested_options = []
                        objects, _ = TypeScriptParser._split_into_objects(options_content)

                        for opt_obj_str in objects:
                            # Recursively parse each nested option as a full property
                            nested_opt = TypeScriptParser._parse_single_property(opt_obj_str)
                            if nested_opt:
                                nested_options.append(nested_opt)
                        prop['options'] = nested_options

                        if prop.get('name') == 'options':
                            print(f"  Final: {len(nested_options)} nested options\n")
                    else:
                        if prop.get('name') == 'options':
                            print(f"  FAILED: depth not 0\n")
                        prop['options'] = []
                else:
                    prop['options'] = []
            else:
                ...
                if bracket_start != -1:
                    ...
                    if depth == 0:
                        ...
                        for opt_obj_str in objects:
                            ...
                            name = TypeScriptParser.extract_string_value(opt_obj_str, 'name')
                            displayName = TypeScriptParser.extract_string_value(opt_obj_str, 'displayName')
                            if name:
                                fixed_opt['name'] = name
                            if displayName:
                                fixed_opt['displayName'] = displayName

                            # Extract values array using bracket-depth counting
                            values_start = opt_obj_str.find('values:')
                            if values_start != -1:
                                values_bracket_start = opt_obj_str.find('[', values_start)
                                if values_bracket_start != -1:
                                    values_depth = 1
                                    j = values_bracket_start + 1
                                    while j < len(opt_obj_str) and values_depth > 0:
                                        if opt_obj_str[j] == '[':
                                            values_depth += 1
                                        elif opt_obj_str[j] == ']':
                                            values_depth -= 1
                                    ...
                                    if values_depth == 0:
                                        values_content = opt_obj_str[values_bracket_start + 1:j - 1]
                                        value_objs = []
                                        value_objects, _ = TypeScriptParser._split_into_objects(values_content)
                                        for val_obj_str in value_objects:
                                            # Recursively parse each value as a property
                                            val_prop = TypeScriptParser._parse_single_property(val_obj_str)
                                            if val_prop:
                                                value_objs.append(val_prop)
                                        fixed_opt['values'] = value_objs
        ...
        if display_opts_match:
            ...
            if depth == 0:
                ...
                # Populate _operation field from displayOptions
                if 'displayOptions' in prop and 'show' in prop['displayOptions'] and 'operation' in prop['displayOptions']['show']:
                    operations = prop['displayOptions']['show']['operation']
                    if isinstance(operations, list) and len(operations) > 0:
                        prop['_operation'] = operations[0] if len(operations) == 1 else ','.join(operations)
...
class NodeExtractor:
    ...
    def __init__(self, node_name: str):
        ...
        self.main_file = None
        self.files = {}
        self.action_file_paths = {}  # Store paths for action files (for spread resolution)
    ...
    def load_files(self) -> bool:
        ...
        # Load action files and store their paths for spread resolution
        actions_dir = self.node_dir / "actions"
        self.action_file_paths = {}  # Store paths for spread resolution
        if actions_dir.exists():
            for action_file in actions_dir.rglob("*.ts"):
                # Generate a unique key for this file based on its relative path
                # This ensures we load ALL .ts files in actions/, not just .operation.ts and .resource.ts
                rel_path = action_file.relative_to(actions_dir)

                # Create a clean key from the path
                # E.g., "message/send.ts" -> "action_message_send"
                # E.g., "versionDescription.ts" -> "action_versionDescription"
                action_key = f"action_{str(rel_path).replace('/', '_').replace('.ts', '')}"

                # Load the file and store its path
                try:
                    with open(action_file, 'r', encoding='utf-8') as f:
                        self.files[action_key] = f.read()
                        self.action_file_paths[action_key] = action_file  # Store the actual file path
                        print(f"  ✓ Loaded action file: {rel_path}")
                except Exception as e:
    ...
    def extract_info(self):
        ...
        if not self.node_info.get('outputs'):
            ...
            if outputs_match:
                ...
                # Check for expression-based outputs: outputs: `={{...}}`
        ...
        for file_name, content in self.files.items():
            if file_name not in ['main', 'properties_file', 'generic_functions']:
                # Use the actual file path for action files (for proper spread resolution)
                # Otherwise use main_file as fallback
                file_path = self.action_file_paths.get(file_name, self.main_file)
                file_props = TypeScriptParser.extract_properties_array(content, file_path)
                if file_props:
                    print(f"  ✓ Extracted {len(file_props)} properties from {file_name}")
                    ...
                    print(f"  ⚠ No properties found in {file_name}")
